...
// ─── P7: Create form — recurrence select + conditional detail fields ───
// The create form in v1.0.18 original has the end field then location.
// We insert the full recurrence block between end and location.
...
patch('P9',
    ...
    'async _handleCreateEvent(){let e=this.shadowRoot.querySelector("#event-title")?.value?.trim(),t=this.shadowRoot.querySelector("#event-calendar")?.value,n=this.shadowRoot.querySelector("#event-start")?.value,i=this.shadowRoot.querySelector("#event-end")?.value,a=this.shadowRoot.querySelector("#event-location")?.value?.trim(),fq=this.shadowRoot.querySelector("#event-recurrence")?.value;if(!e||!n)return;let r=eh.DateTime.fromISO(n),s=i?eh.DateTime.fromISO(i):r.plus({hours:1}),rr="";if(fq){let iv=parseInt(this.shadowRoot.querySelector("#event-recurrence-interval")?.value)||1,bd=[];if(fq==="FREQ=WEEKLY"){bd=[...this.shadowRoot.querySelectorAll("#event-day-picker .day-btn.active")].map(b=>b.dataset.day)}let md=fq==="FREQ=MONTHLY"?parseInt(this.shadowRoot.querySelector("#event-recurrence-monthday")?.value):null,et=this.shadowRoot.querySelector("#event-recurrence-end")?.value||"never",ed=this.shadowRoot.querySelector("#event-recurrence-end-date")?.value||"",ec=parseInt(this.shadowRoot.querySelector("#event-recurrence-end-count")?.value)||10;rr=this._buildRrule(fq,iv,bd,md,et,ed,ec)}let o={entity_id:t,summary:e,start_date_time:r.toFormat("yyyy-MM-dd HH:mm:ss"),end_date_time:s.toFormat("yyyy-MM-dd HH:mm:ss")};a&&(o.location=a);rr&&(o.rrule=rr);try{await this.hass.callService("calendar","create_event",o)'
);
